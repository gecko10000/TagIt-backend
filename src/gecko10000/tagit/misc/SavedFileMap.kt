...
class SavedFileMap : ConcurrentHashMap<UUID, SavedFile>() {
    ...
    override fun put(key: UUID, value: SavedFile): SavedFile? {
        putListeners.map { it(value) }
        return super.put(key, value)
    }

    override fun remove(key: UUID): SavedFile? {
        val savedFile = super.remove(key)
        savedFile?.let { removeListeners.map { it(savedFile) } }
        return savedFile
    }

    override fun remove(key: UUID, value: SavedFile): Boolean {
